...
public class BlogController(IBlogService blog, IValidator<CreateBlogCommentDto> validator) : ApiControllerBase
{
    ...
    [HttpPost("{slug}/comments")] public async Task<ActionResult<ApiResponse<BlogCommentDto>>> Comment(string slug,CreateBlogCommentDto dto,CancellationToken ct) { await ValidateAsync(validator,dto,ct); var result=await blog.AddCommentAsync(slug,dto,User.FindFirstValue(ClaimTypes.NameIdentifier),ct); return StatusCode(201,ApiResponse<BlogCommentDto>.Ok(result,"Comment submitted for moderation.")); }
}
