...
public class AdminBlogController(IBlogService blog,IFileStorageService files,IValidator<CreateBlogPostDto> createValidator,IValidator<UpdateBlogPostDto> updateValidator) : ApiControllerBase
{
    ...
    [HttpPost("cover")] public async Task<ActionResult<ApiResponse<string>>> Cover(IFormFile cover,CancellationToken ct){files.Validate(cover.OpenReadStream(),cover.FileName,cover.Length);await using var stream=cover.OpenReadStream();return Ok(ApiResponse<string>.Ok(await files.SaveAsync(stream,cover.FileName,"blog",ct)));}
    ...
    [HttpPatch("comments/{id:int}")] public async Task<ActionResult<ApiResponse<BlogCommentDto>>> Moderate(int id,ModerateCommentDto dto,CancellationToken ct)=>Ok(ApiResponse<BlogCommentDto>.Ok(await blog.ModerateCommentAsync(id,dto.Status,ct)));
